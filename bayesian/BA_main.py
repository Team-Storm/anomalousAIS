...
#PROCESS data
dict_long, dict_lat,dict_deltime,usermap  = load_data(pd.read_csv("bayesdata.csv"))
...
#pickle.dump((long_arr, lat_arr, ids, usermap), open('%s_bucketed.p' % (dataname), 'wb'))

#LOAD processed data
#long_arr, lat_arr, ids, usermap = pickle.load(open('%s_bucketed.p' % (dataname), 'rb'))
...
plt.savefig('AIS_lat_goodbad.png')
...
plt.savefig('AIS_long_goodbad.png')
...
plt.savefig('Distribution_all_ships_lat_only.png')
...
plt.ylabel('Frequency')
plt.savefig('Distribution_all_ships_long_only.png')