...
class Cluster:  
    def __init__(self):
        self.cluster = [] # List of trajectory points
        self.avgCOG = 0.0
    def calculateAvgDirection(self):
        avgDirection = 0.0
        for i in range(len(self.cluster)):
            avgDirection += self.cluster[i].COG
...
class DBScanSD:
    ...
    def mergeCluster( self , clusterA , clusterB):
        ...
        if(len(clusterA.cluster)==0 and len(clusterB.cluster)==0):
            return merge
        for index in range(len(clusterB.cluster)):
            p = clusterB.cluster[index]
            if(p.isCorePoint and p in clusterA.cluster):
                merge = True
                for j in range(len(clusterB.cluster)):
                    clusterA.cluster.append(clusterB.cluster[j])
                break
        return merge
...
class GravityVectorExtraction:
    ...
    def extractGravityVector(self,cluster):
        ...
        for i in range(len(cluster.cluster)):
            mp = MappingPoint.convertPointToMappingPoint(cluster.cluster[i],avgCOG)
            mpLst.append(mp)
        ...
        traPointsTMP = []
        meanDistance=0
        #Partition the points and calculate GV for each cell
        while count <= len(mpLst):
            if (count<len(mpLst) and (mpLst[count].mappingtude - mpLst[k].mappingtude)<0.01): #0.01 as the predefined grid width
                ...
                for i in range(len(traPointsTMP)):
                    ...
                    dist = self.gpsDistance(lat,lon,y,x)
    ...
    def insertionSort(self,mpl):
      ...
      ## Retrieve the quartile value from an array ,used for generating relative distance.
